...
test("switching to 中文 repaints in place and keeps the basket", () => {
  // Put one Focaccia in the basket (RM15) through the real stepper.
  firstCardStepper().children[2]._listeners.click[0]();
  assert.equal(barTotal(), "RM15.00", "the basket has one Focaccia");
  ...
  assert.equal(barTotal(), "RM15.00", "the basket survived the switch");
...
test("switching 中文 → BM relabels the page and the order button", () => {
  ...
  assert.equal(barTotal(), "RM15.00", "the basket is still there");
});
...
test("the track card repaints in the chosen language with no network", async () => {
  ...
  assert.equal(barTotal(), "RM15.00", "the basket is untouched by every switch");
});
...
test("the language pills are wired to the in-place switch", () => {
  ...
  assert.equal(barTotal(), "RM15.00", "the basket is still intact after a pill tap");
});
