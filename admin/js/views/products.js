...
const FIELD_LABEL = { name: "Name", description: "Description", unit: "Selling unit", servingTip: "Serving tip" };
...
function buildEditor(state, product) {
  ...
  const recipeCard = el("div", { class: "card" },
    ...
    el("p", { class: "card-sub", style: "margin:0 0 8px" },
      "Type the ingredients… or pick another product to make a set (e.g. 4 × Focaccia) — its own recipe is used automatically. Under each line is how its cost is counted (amount × price); the list below adds the lines up and shows each one's share of the total. Each ingredient line can also carry a short note about itself for this product only (e.g. which flour), written for you alone — never shown to customers."),
    costEl,
...
function editorFields(state, editor) {
  return el("div", {},
    ...
    el("div", { class: "field" }, el("label", {}, "Description (customers read it on your shop)"),
      ...
      editor.desc),
    el("div", { class: "field" }, el("label", {}, "Serving tip (sent in your follow-up message)"),
      el("p", { class: "card-sub", style: "margin:0 0 5px" },
...
function recipeLine(state, line, i, draft, refresh, selfId, cost) {
  ...
  // A private per-product note under the ingredient line — "which flour is this?"
  // Written separately for each product, so the same ingredient can read
  // differently here and there. Never published to the shop (cleanStorefront
  // drops recipes entirely) and never on a label.
