...
export function renderCustomers(root, state, params) {
  ...
  const finder = el("input", {
    ...
    placeholder: "Find a customer — name, number, dog, likes…",
  ...
  function drawList() {
    ...
    if (!rows.length) {
      finderCount.textContent = "";
      listBox.replaceChildren(emptyState("Nothing found", `No customer matches “${query}” — try their name, number or dog's name.`));
      return;
    }
      ...
      el("div", { class: "profile-who" },
        ...
        ...(empty
          ? [el("p", { class: "card-sub", style: "margin:2px 0 0" },
              "Get to know this customer — dog's name, what they like, what to avoid.")]
          : facts)),
...
function editProfilePopup(state, r, afterSave) {
  ...
  const likes = el("input", { class: "input", value: p.likes || "", placeholder: "e.g. banana, extra cocoa" });
  const avoid = el("input", { class: "input", value: p.avoid || "", placeholder: "e.g. nuts, coconut" });
  ...
  const draw = () => {
    preview.replaceChildren(
      ...
      photo
        ...
        : button("Choose dog photo", () => file.click(), "soft small"),
  ...
  showPopup(`Profile — ${(p.dogName || r.name || "this customer")}`, (refresh, closeFn) => {
    ...
    return el("div", {},
      ...
      el("div", { class: "field" }, el("label", {}, "🐕 Dog's name"), dogName),
