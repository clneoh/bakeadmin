...
function pickerCard(state, metas, ticked, toggle, open, toggleOpen, onIgnore, onCover) {
  ...
  return el("div", { class: "card" },
    ...
    el("p", { class: "card-sub", style: "margin:0 0 4px" },
      "Tick the bake days to shop for — their ingredient needs add up into ONE list. Days you've already saved stay unticked; a day marked \"orders changed\" has a new order since you shopped — tap it to review. Extra orders keep adding onto that day's one open list until you save it."),
...
function previewCard(state, chosen, bom, needsReview) {
  ...
  // top-up even when today's bakes don't use it ("add any low one"). The sweep
