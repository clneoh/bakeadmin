...
export function renderSettings(root, state) {
  ...
  const sfName = el("input", { class: "input", value: sf.name, placeholder: "Jienluv2bake", "data-suggest": "Jienluv2bake",
  ...
  const sfTagline = el("input", { class: "input", value: sf.tagline, placeholder: "Home-made focaccia & sandwiches", "data-suggest": "Home-made focaccia & sandwiches",
    onchange: () => { sf.tagline = sfTagline.value.trim(); save(state); maybeSyncStorefront(state); } });
  const sfInsta = el("input", { class: "input", value: sf.instagram, placeholder: "e.g. jienluv2bake", "data-suggest": "jienluv2bake",
    onchange: () => { sf.instagram = sfInsta.value.trim(); save(state); maybeSyncStorefront(state); } });
  const sfFacebook = el("input", { class: "input", value: sf.facebook, placeholder: "e.g. jienluv2bake", "data-suggest": "jienluv2bake",
  ...
  const sfPolicy = el("textarea", { class: "input", rows: 3, value: sf.policy || "",
    placeholder: "e.g. Orders are not refundable, but you may move your order to another delivery day up to 2 days before it is due.",
...
function exportState(state) {
  ...
  downloadJSON(`jienluv2bake-backup-${todayISO()}.json`, JSON.stringify(payload, null, 2));
}
